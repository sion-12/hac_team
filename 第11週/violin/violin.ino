...
// ↑ これはコントローラ(シリアル)が送ってくるノート番号→周波数の対応表。
//   バイオリン本来の最低音は G3=196Hz なので、先頭の 185Hz は実楽器より少し低い。
//   よりバイオリンらしい音域にしたいときは、表全体を 1 オクターブ上げる(各値×2)などで調整。

float volume = 0.8;

// ===== バイオリン音色パラメータ ======================================
float BRIGHTNESS = 3000;        // 高域ロールオフ[Hz]。バイオリンは明るめなので高め
// =====================================================================

// ビブラート (LFOでサンプル単位に揺らす) ※最初のバイオリンコードに合わせた値
float vibratoRate  = 5.5;       // 揺れの速さ [Hz]
float vibratoDepth = 0.005;     // ピッチ変動の割合 (±0.5%。バイオリンらしい繊細な揺れ)

// 音量カーブ (1音の中の膨らみ = 中→大)
float fadeIn     = 0.05;        // 立ち上がり (プチノイズ防止)
float fadeOut    = 0.12;        // 終わり    (プチノイズ防止)
float swellDepth = 0.20;        // 中→大の振り幅 (0=平坦 / 大きいほど出だしが小さく終わりが大きい)

// 弓の食いつき (弾いている感)
float attackBite = 2.5;         // 立ち上がりの鋭さ (1=直線 / 大きいほどキュッと速い)
float bowGrip    = 0.18;        // 弾き始めの擦過ノイズ量 (0=なし)
float gripMs     = 60;          // 擦過ノイズが出る時間 [ms]

// 発音の粒立ち (弓を返す感じ)
float gateRatio  = 0.9;         // 音符の長さに対する発音割合 (1=繋がる / 小さいほど切れる)
float minGapMs   = 40;          // 音と音の最小すき間 [ms]

float noise = 0.001;

// --- 音声グラフ ------------------------------------------------------
//  Oscil(音色) → [周波数にLFOビブラート] → BowEnv(大中大の音量) → Reverb → out
...
void setup() {
  ...
  for (int i = 0; i < frequencies.length; i++) {
    tables[i] = makeViolinTable(frequencies[i]);
  }

  // --- 音声グラフを組む ---
  vibBase    = new Constant(frequencies[0]);
  vibratoLFO = new Oscil(vibratoRate, frequencies[0] * vibratoDepth, Waves.SINE);
  freqSum    = new Summer();
  bowEnv     = new BowEnv();
  reverb     = new Reverb();
...
// =====================================================================
//  makeViolinTable: バイオリン音色の単一周期ウェーブテーブルを作る
//   各倍音の振幅 = (1/h) * ボディ共鳴(h*freq) * 高域ロールオフ
//   ※倍音数の式・上限は最初のバイオリンコード(11000/freq, 12〜60本)に準拠
// =====================================================================
Wavetable makeViolinTable(float freq) {
  int N = 1024;
  float[] wave = new float[N];

  int numH = constrain((int)(11000.0 / freq), 12, 60);
...
// =====================================================================
//  ボディ共鳴 (フォルマント): バイオリン胴体の共鳴特性
//   ※最初のバイオリンコードのフォルマント配置をそのまま移植
// =====================================================================
float bodyResonance(float f) {
  float g = 0.25;
  g += resonancePeak(f,  280,  90, 1.0);
  g += resonancePeak(f,  460, 110, 1.3);   // 主共鳴 (暖かさ・芯)
  g += resonancePeak(f,  700, 150, 0.7);
  g += resonancePeak(f, 1200, 400, 0.5);
  g += resonancePeak(f, 2800, 900, 0.5);   // 高音のつや
  return g;
}
...
//   ※定数は最初のバイオリンコードの残響に合わせた。残響を切りたいときは wet = 0
// =====================================================================
class Reverb extends UGen {
  ...
  float dry = 0.75, wet = 0.35, inGain = 0.4;
