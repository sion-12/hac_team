...
// ===== チェロ音色パラメータ ==========================================
float BRIGHTNESS = 2000;        // 高域ロールオフ[Hz]。低いほど暗く太い
// =====================================================================

// ビブラート (LFOでサンプル単位に揺らす)
float vibratoRate  = 5.2;       // 揺れの速さ [Hz]
float vibratoDepth = 0.013;     // ピッチ変動の割合 (±1.3%。弦らしく明確に)

// 音量カーブ (1音の中の膨らみ = 中→大)
float fadeIn     = 0.05;        // 立ち上がり (プチノイズ防止)
float fadeOut    = 0.12;        // 終わり    (プチノイズ防止)
float swellDepth = 0.30;        // 中→大の振り幅 (0=平坦 / 大きいほど出だしが小さく終わりが大きい)
...
// 発音の粒立ち (弓を返す感じ)
float gateRatio  = 0.88;         // 音符の長さに対する発音割合 (1=繋がる / 小さいほど切れる)
...
void setup() {
  ...
  for (int i = 0; i < frequencies.length; i++) {
    tables[i] = makeCelloTable(frequencies[i]);
  }
...
// =====================================================================
//  makeCelloTable: チェロ音色の単一周期ウェーブテーブルを作る
//   各倍音の振幅 = (1/h) * ボディ共鳴(h*freq) * 高域ロールオフ
// =====================================================================
Wavetable makeCelloTable(float freq) {
  int N = 1024;
  float[] wave = new float[N];

  int numH = constrain((int)(9000.0 / freq), 16, 90);
...
// =====================================================================
//  ボディ共鳴 (フォルマント): チェロ胴体の共鳴特性
// =====================================================================
float bodyResonance(float f) {
  float g = 0.22;
  g += resonancePeak(f,  100,  45, 1.0);   // 空気共鳴 (A0)
  g += resonancePeak(f,  175,  55, 1.4);   // 主木部共鳴 (暖かさの芯)
  g += resonancePeak(f,  250,  80, 1.0);
  g += resonancePeak(f,  400, 150, 0.7);   // 中音域の豊かさ
  g += resonancePeak(f,  900, 300, 0.5);
  g += resonancePeak(f, 1600, 600, 0.45);  // 高音のつや
  return g;
}
